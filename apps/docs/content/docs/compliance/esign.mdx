...
## How OneMed Sign Supports Compliance

OneMed Sign provides features that support compliance with e-signature laws across jurisdictions:
...
OneMed Sign maintains an audit trail for each document, recording:
...
## What OneMed Sign Does NOT Provide

OneMed Sign supports compliance with Simple Electronic Signature (SES) requirements. The following are not currently provided:
