...
- A OneMed Sign account on any plan (Free, Individual, Team, or Enterprise)

<Callout type="info">
  Free accounts include API access with a limit of 5 documents per month. [Upgrade to a paid
  plan](https://onemed.com.sg/pricing) for higher limits.
</Callout>
...
- Log in to your OneMed Sign account
...
```bash
curl https://app.onemed.com.sg/api/v2/documents \
...
```typescript
const response = await fetch('https://app.onemed.com.sg/api/v2/documents', {
  method: 'GET',
  headers: {
...
OneMed Sign provides official SDKs that handle authentication for you:

```typescript
import { OneMed Sign } from '@documenso/sdk-typescript';

const client = new OneMed Sign({
...
<Callout type="info">
  ...
  OpenAPI reference](https://app.onemed.com.sg/api/v1/openapi).
</Callout>
