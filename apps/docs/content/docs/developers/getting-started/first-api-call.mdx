...
description: Create and send a document for signing using the OneMed Sign API, from uploading a PDF to adding recipients and distributing for signature.
---
...
- A OneMed Sign account (cloud or self-hosted)
...
| ----------- | -------------------------------------- |
| Production  | `https://app.onemed.com.sg/api/v2`     |
| Staging     | `https://stg-app.onemed.com.sg/api/v2` |
...
<Tab value="curl">
```bash
curl -X POST "https://app.onemed.com.sg/api/v2/envelope/envelope_abc123/distribute" \
  -H "Authorization: YOUR_API_TOKEN" \
...
const response = await fetch(
  `https://app.onemed.com.sg/api/v2/envelope/${envelopeId}/distribute`,
  {
    method: 'POST',
    headers: {
      Authorization: 'YOUR_API_TOKEN',
...
BASE_URL="https://app.onemed.com.sg/api/v2"
...
- [API Reference](https://openapi.onemed.com.sg/) - Full endpoint documentation with request/response schemas
