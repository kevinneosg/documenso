...
Webhooks are HTTP callbacks triggered by specific events in OneMed Sign. When an event occurs (such as a document being signed), OneMed Sign sends an HTTP POST request to your configured URL with details about the event.
...
Before configuring a webhook in OneMed Sign, you need an endpoint that can receive HTTP POST requests. Here's a minimal example:
...
<Callout type="warn">
  Always respond with a `200 OK` status within 30 seconds. OneMed Sign will retry failed deliveries.
</Callout>

## Configuring Webhooks in OneMed Sign via the Dashboard
...
OneMed Sign provides a built-in testing feature to verify your webhook endpoint works correctly.
...
<Step>
OneMed Sign will attempt to deliver the same payload again
</Step>
</Steps>

## Retry Policy

When a webhook delivery fails (non-2xx response or timeout), OneMed Sign automatically retries with exponential backoff:
...
<Accordions type="multiple">
  ...
  <Accordion title="Verify signatures">
    Use a webhook secret and verify the `X-OneMed Sign-Secret` header (see [Verification](/docs/developers/webhooks/verification)).
  </Accordion>
