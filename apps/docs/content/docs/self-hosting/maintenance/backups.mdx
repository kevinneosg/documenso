---
title: Backups
description: Backup strategies for OneMed Sign data.
---
...
OneMed Sign stores data in three locations:
...
Create a script to back up all OneMed Sign components:
...
echo "Starting OneMed Sign backup: $TIMESTAMP"
...
```ini
[Unit]
Description=OneMed Sign Backup
...
```ini
[Unit]
Description=Daily OneMed Sign Backup
