...
description: Configure PostgreSQL connection strings, pooling, SSL, migrations, and performance tuning for your OneMed Sign deployment.
---
...
OneMed Sign requires **PostgreSQL 14 or later**. No other databases are supported.
...
OneMed Sign uses two database connection variables:
...
OneMed Sign automatically detects common database environment variable formats used by hosting providers:
...
If your hosting provider sets these variables, OneMed Sign will use them automatically.
...
OneMed Sign uses Prisma, which maintains its own connection pool. Configure pool size with connection string parameters:
...
Database migrations update your schema when upgrading OneMed Sign.

### Automatic Migrations

When running OneMed Sign via Docker, migrations run automatically on container startup. No manual intervention is required.
...
Key parameters for OneMed Sign workloads:
...
OneMed Sign includes necessary indexes by default. Additional indexes may help for:
