...
OneMed Sign supports four email transport methods:
...
SMTP is the most common email transport. OneMed Sign supports two SMTP authentication methods.
...
Generate an API key in the Resend dashboard for OneMed Sign to use.
...
OneMed Sign uses React Email templates for all outgoing emails. The templates are located in `packages/email/templates/` and include:
...
- Custom logo replaces the OneMed Sign logo
...
Configure OneMed Sign to use the test server:
...
NEXT_PRIVATE_SMTP_FROM_NAME="OneMed Sign Test"
```
...
For MailChannels, configure DKIM directly in OneMed Sign using the environment variables described above.
