...
description: Complete reference for all environment variables used to configure a self-hosted OneMed Sign instance.
---

## Required Variables

These variables must be set for OneMed Sign to function:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL of your OneMed Sign instance (e.g., `https://sign.example.com`)                   |
...
OneMed Sign requires PostgreSQL 14 or higher.
...
OneMed Sign supports multiple email transports for sending notifications.
...
OneMed Sign can store documents in the database or S3-compatible storage.
...
OneMed Sign requires a certificate to digitally sign documents.
...
OneMed Sign can use Google Vertex AI for recipient and field detection.
...
OneMed Sign supports multiple background job providers for processing emails, documents, webhooks, and scheduled tasks.
...
These variables require an active [Enterprise Edition](/docs/policies/enterprise-edition) license. Obtain a license key from [license.onemed.com.sg](https://license.onemed.com.sg) and set it below to unlock enterprise features such as SSO, embed authoring, and 21 CFR Part 11 compliance.
