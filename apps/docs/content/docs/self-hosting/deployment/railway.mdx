---
title: Railway
description: Deploy OneMed Sign on Railway with automatic builds from Git, managed PostgreSQL, and built-in networking. Use the one-click template or manual setup.
---
...
Click the URL in the Railway dashboard to access your OneMed Sign instance.
...
If you prefer more control over the setup, deploy OneMed Sign manually:
...
- Fork or select the [OneMed Sign repository](https://github.com/documenso/documenso)
...
Click on the OneMed Sign service and go to the "Variables" tab. Add the following variables:
...
NEXT_PRIVATE_SMTP_FROM_NAME=OneMed Sign
...
OneMed Sign runs database migrations automatically on startup.
...
- Click on your OneMed Sign service
...
Railway provides several options for scaling your OneMed Sign deployment:
...
<Steps>
<Step>
### Open your OneMed Sign service

In the Railway dashboard, select the OneMed Sign service.
...
In the Railway dashboard, select your OneMed Sign service and open "Settings".
...
<Callout type="warn">
  OneMed Sign stores sessions in the database, so horizontal scaling works without additional
...
Railway uses usage-based pricing. Estimated monthly costs for OneMed Sign:
...
OneMed Sign requires a signing certificate to sign documents. On Railway, you can:

### Option 1: Use Generated Certificate (Default)

OneMed Sign generates a self-signed certificate automatically if none is provided. This works for testing but shows generic certificate information in signed documents.
...
In your OneMed Sign service variables, set:
