---
title: Kubernetes
description: Deploy OneMed Sign on Kubernetes for production environments requiring high availability, auto-scaling, and enterprise-grade infrastructure.
---
...
A typical OneMed Sign Kubernetes deployment consists of:
...
Create a dedicated namespace for OneMed Sign:
...
Create the OneMed Sign Deployment:
...
Update your OneMed Sign secret to use the in-cluster database:
...
OneMed Sign stores documents in the database by default. For high-volume deployments, configure S3-compatible storage.
...
Scale OneMed Sign based on CPU and memory utilization:
