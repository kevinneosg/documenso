---
title: Docker
description: Run OneMed Sign as a standalone Docker container with an external database. Ideal for container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.
---
...
These variables must be set for OneMed Sign to start:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL where OneMed Sign is accessible (e.g., `https://sign.example.com`) | -                             |
...
OneMed Sign requires two database connection strings:
...
docker run -d \
  ...
  -e NEXT_PRIVATE_SMTP_FROM_NAME="OneMed Sign" \
...
OneMed Sign provides health check endpoints for monitoring:
...
To update OneMed Sign to a new version:
...
The OneMed Sign container is stateless. All persistent data is stored in the PostgreSQL database.
